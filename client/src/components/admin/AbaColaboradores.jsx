// =============================================================================
// Aba de Colaboradores — CRUD de colaboradores
...
export default function AbaColaboradores({ colaboradores, busca, setBusca, filtroFuncao, setFiltroFuncao, onRecarregar, modalColab, setModalColab }) {
  ...
  async function deletarColab(id) {
    ...
    try {
      await colaboradoresAPI.deletar(id);
      onRecarregar();
    } catch (err) {
  ...
  const funcoesUnicas = [...new Set(colaboradores.map(c => c.funcao))].sort();
  ...
  return (
    <div>
              ...
              <tr key={colab.id}>
                ...
                <td>
                  <div className="admin-acoes">
                    <button className="admin-acao-btn" onClick={() => abrirEditar(colab)} title="Editar">✏️</button>
                    <button className="admin-acao-btn perigo" onClick={() => deletarColab(colab.id)} title="Deletar">🗑</button>
                  </div>
                </td>
              </tr>
            ))}
            {colabsFiltrados.length === 0 && (
              <tr><td colSpan={7} style={{ textAlign: 'center', padding: '32px', color: 'var(--cor-texto-terciario)' }}>Nenhum colaborador encontrado</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <Modal
        ...
        rodape={
          <>
            <button className="btn btn-fantasma" onClick={() => setModalColab({ aberto: false, colab: null })}>Cancelar</button>
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div className="campo-grupo">
            ...
            <input className="campo-input" value={formColab.nome} onChange={e => setFormColab({...formColab, nome: e.target.value})} />
          </div>
          <div className="campo-grupo">
            <label className="campo-label">Função</label>
            <input className="campo-input" value={formColab.funcao} onChange={e => setFormColab({...formColab, funcao: e.target.value})} />
          </div>
          <div className="campo-grupo">
            <label className="campo-label">Idade</label>
            <input className="campo-input" type="number" value={formColab.idade} onChange={e => setFormColab({...formColab, idade: e.target.value})} />
          </div>
          <div className="campo-grupo">
            <label className="campo-label">Gênero</label>
            <select className="campo-select" value={formColab.genero} onChange={e => setFormColab({...formColab, genero: e.target.value})}>
          ...
          <div className="campo-grupo">
            ...
            <input className="campo-input" value={formColab.celular} onChange={e => setFormColab({...formColab, celular: e.target.value})} />
          </div>
          <div className="campo-grupo">
            <label className="campo-label">Email</label>
            <input className="campo-input" value={formColab.email} onChange={e => setFormColab({...formColab, email: e.target.value})} />
          </div>
        </div>
        <div className="campo-grupo">
          <label className="campo-label">Endereço</label>
          <input className="campo-input" value={formColab.endereco} onChange={e => setFormColab({...formColab, endereco: e.target.value})} />
        </div>
      </Modal>
    </div>
  );
